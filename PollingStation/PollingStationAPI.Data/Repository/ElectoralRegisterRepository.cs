...
public class ElectoralRegisterRepository : IRepository<RegisteredVoter, Guid>
{
    private readonly Container _container;

    // Constants for database and container names (as in your Terraform)
    private const string DatabaseName = "VotingDatabase";
    private const string ContainerName = "ElectoralRegister";

    public ElectoralRegisterRepository(CosmosClient cosmosClient)
    {
    ...
    public async Task Add(RegisteredVoter entity)
    {
        if (entity == null)
    ...
    public async Task<bool> Delete(Guid entityId)
    {
        ...
        try
        {
            ...
            await _container.DeleteItemAsync<RegisteredVoter>(cosmosId, new PartitionKey(cosmosId));
            return true;
        }
    ...
    public async Task<RegisteredVoter?> GetById(Guid entityId)
    {
        string cosmosId = entityId.ToString(); // Convert int to string for Cosmos DB ID
        ...
        try
        {
            ...
            ItemResponse<RegisteredVoter> response = await _container.ReadItemAsync<RegisteredVoter>(cosmosId, new PartitionKey(cosmosId));
    ...
    public async Task<RegisteredVoter?> Update(RegisteredVoter entity)
    {
        if (entity == null)
            throw new ArgumentNullException(nameof(entity));
        ...
        try
        {
            ...
            ItemResponse<RegisteredVoter> response = await _container.ReplaceItemAsync(entity, entity.Id.ToString(), new PartitionKey(entity.Id.ToString()));
            return response.Resource;
        }
        catch (CosmosException ex) when (ex.StatusCode == HttpStatusCode.NotFound)
        {
    ...
    public async Task<IEnumerable<RegisteredVoter>> Filter(Expression<Func<RegisteredVoter, bool>> predicate)
    {
        var queryable = _container.GetItemLinqQueryable<RegisteredVoter>(allowSynchronousQueryExecution: false);
        var filteredQuery = queryable.Where(predicate).ToFeedIterator();

        var results = new List<RegisteredVoter>();
