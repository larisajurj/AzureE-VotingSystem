...
public class CommitteeMemberRepository : IRepository<CommitteeMember, string>
{
    private readonly Container _container;

    // Constants for database and container names (as in your Terraform)
    private const string DatabaseName = "VotingDatabase";
    private const string ContainerName = "CommitteeMember";

    public CommitteeMemberRepository(CosmosClient cosmosClient)
    {
    ...
    public async Task Add(CommitteeMember entity)
    {
        if (entity == null)
    ...
    public async Task<bool> Delete(string entityId)
    {
        ...
        try
        {
            ...
            await _container.DeleteItemAsync<CommitteeMember>(cosmosId, new PartitionKey(cosmosId));
            return true;
        }
    ...
    public async Task<CommitteeMember?> GetById(string entityId)
    {
        string cosmosId = entityId.ToString(); // Convert int to string for Cosmos DB ID
        ...
        try
        {
            ...
            ItemResponse<CommitteeMember> response = await _container.ReadItemAsync<CommitteeMember>(cosmosId, new PartitionKey(cosmosId));
    ...
    public async Task<CommitteeMember?> Update(CommitteeMember entity)
    {
        if (entity == null)
            throw new ArgumentNullException(nameof(entity));
        ...
        try
        {
            ...
            ItemResponse<CommitteeMember> response = await _container.ReplaceItemAsync(entity, entity.Id.ToString(), new PartitionKey(entity.Id.ToString()));
            return response.Resource;
        }
        catch (CosmosException ex) when (ex.StatusCode == HttpStatusCode.NotFound)
        {
    ...
    public async Task<IEnumerable<CommitteeMember>> Filter(Expression<Func<CommitteeMember, bool>> predicate)
    {
        var queryable = _container.GetItemLinqQueryable<CommitteeMember>(allowSynchronousQueryExecution: false);
        var filteredQuery = queryable.Where(predicate).ToFeedIterator();

        var results = new List<CommitteeMember>();
