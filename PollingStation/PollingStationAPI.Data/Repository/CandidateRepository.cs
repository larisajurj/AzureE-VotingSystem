...
public class CandidateRepository : IRepository<Candidate, Guid>
{
    private readonly Container _container;

    private const string DatabaseName = "VotingDatabase";
    private const string ContainerName = "Candidate";

    public CandidateRepository(CosmosClient cosmosClient)
    {
    ...
    public async Task<bool> Delete(Guid entityId)
    {
        ...
        try
        {
            ...
            await _container.DeleteItemAsync<Candidate>(cosmosId, new PartitionKey(cosmosId));
            return true;
        }
    ...
    public async Task<Candidate?> GetById(Guid entityId)
    {
        string cosmosId = entityId.ToString(); // Convert int to string for Cosmos DB ID
        ...
        try
        {
            ...
            ItemResponse<Candidate> response = await _container.ReadItemAsync<Candidate>(cosmosId, new PartitionKey(cosmosId));
    ...
    public async Task<Candidate?> Update(Candidate entity)
    {
        ...
        try
        {
            ...
            ItemResponse<Candidate> response = await _container.ReplaceItemAsync(entity, entity.Id.ToString(), new PartitionKey(entity.Id.ToString()));
            return response.Resource;
        }
        catch (CosmosException ex) when (ex.StatusCode == HttpStatusCode.NotFound)
        {
    ...
    public async Task<IEnumerable<Candidate>> Filter(Expression<Func<Candidate, bool>> predicate)
    {
        var queryable = _container.GetItemLinqQueryable<Candidate>(allowSynchronousQueryExecution: false);
